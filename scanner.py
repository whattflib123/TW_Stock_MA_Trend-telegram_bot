...
def fmt_threshold_pct(ratio: Optional[float], threshold: float) -> str:
    if ratio is None:
        return f"N/A / 門檻 {threshold * 100:.2f}%"
    operator = "<" if ratio < threshold else ">="
    return f"{ratio * 100:.2f}% {operator} {threshold * 100:.2f}%"


def fmt_drop_threshold_pct(ratio: Optional[float], threshold: float) -> str:
    if ratio is None:
        return f"N/A / 門檻 -{threshold * 100:.2f}%"
    threshold_ratio = -threshold
    operator = "<" if ratio < threshold_ratio else ">="
    return f"{ratio * 100:.2f}% {operator} {threshold_ratio * 100:.2f}%"


def rsi_status_emoji(rsi_value: Optional[float]) -> str:
...
def rsi_status_label(rsi_value: Optional[float]) -> str:
    if rsi_value is None:
        return "正常"
    if rsi_value < 30:
        return "低位"
...
def rsi_position_text(rsi_value: Optional[float], threshold: float = 30.0) -> str:
    if rsi_value is None:
        return f"RSI: N/A / 門檻 {threshold:.0f}"
    if rsi_value < threshold:
        return f"RSI: 跌破{threshold:.0f}({rsi_value:.2f} < {threshold:.0f})"
    return f"RSI: 站上{threshold:.0f}({rsi_value:.2f} >= {threshold:.0f})"


def drawdown_from_high(close_series: pd.Series) -> Optional[float]:
    if close_series.empty:
        return None
    historical_high = float(close_series.max())
    latest_close = float(close_series.iloc[-1])
    if historical_high <= 0:
        return None
    return (historical_high - latest_close) / historical_high


def high_position_emoji(drawdown_ratio: Optional[float]) -> str:
...
def high_position_label(drawdown_ratio: Optional[float]) -> str:
    if drawdown_ratio is None:
        return "正常"
    if drawdown_ratio < 0.05:
        return "高位"
...
def ma_position_text(close_price: float, ma_value: Optional[float], ma_window: int) -> str:
    if ma_value is None:
        return f"MA{ma_window}: N/A"
    if close_price < ma_value:
        return f"MA{ma_window}: 跌破({close_price:.2f} < {ma_value:.2f})"
    return f"MA{ma_window}: 站上({close_price:.2f} >= {ma_value:.2f})"


def hit_status_emoji(hit: bool) -> str:
    return "🟢" if hit else "⚪"
...
def main() -> None:
    ...
    for stock in config.stocks:
        try:
            ...
            high_drawdown = drawdown_from_high(close_series)
            ...
            rsi_emoji = rsi_status_emoji(rsi_value)
            rsi_label = rsi_status_label(rsi_value)
            high_drawdown_text = fmt_pct(high_drawdown)
            high_position = high_position_emoji(high_drawdown)
